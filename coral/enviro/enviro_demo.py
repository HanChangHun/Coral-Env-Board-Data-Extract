...
def update_display(display, msg, font):
    # Rotated Version
    canvas = Image.new('1', (display.width, display.height), color=0)
    draw = ImageDraw.Draw(canvas)
    draw.text((0, 0), msg, fill=1, font=font)
    rotated_canvas = canvas.rotate(180)
    display.display(rotated_canvas)

    # Normal Version
    # with canvas(display) as draw:
    #     draw.text((0, 0), msg, fill='white', font=font)
...
def main():
    ...
    with CloudIot(args.cloud_config) as cloud:
        ...
        for read_count in itertools.count():
            ...
            update_display(enviro.display, msg, font)
